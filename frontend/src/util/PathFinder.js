...
export class PathFinder {
    ...
    progressPathFinding = destination => {
        ...
        this.currentPathfindingProgress = flatMap(
            this.currentPathfindingProgress,
            pathProgress => {
                ...
                const lastPoint = [
                    pathProgress.path[pathProgress.path.length - 1].b[0],
                    pathProgress.path[pathProgress.path.length - 1].b[1],
                ] // [x,y]
                ...
                const newPaths = flatMap(adjacentEdges, adjacentEdge => {
                    ...
                    if (
                        this.edgeAlreadyInPath(pathProgress.path, adjacentEdge)
                    ) {
                        return []
                    }
                    ...
                    return {
                        ...
                        reachedDestination:
                            newPathEdges[newPathEdges.length - 1].b[0] ===
                                destination[0] &&
                            newPathEdges[newPathEdges.length - 1].b[1] ===
                                destination[1],
                    }
                })
    ...
    edgeAlreadyInPath = (path, requiredEdge) => {
        const foundEdge = find(path, edge => {
            const edgeFound =
                (edge.a[0] === requiredEdge.a[0] &&
                    edge.a[1] === requiredEdge.a[1] &&
                    edge.b[0] === requiredEdge.b[0] &&
                    edge.b[1] === requiredEdge.b[1]) ||
                (edge.a[0] === requiredEdge.b[0] &&
                    edge.a[1] === requiredEdge.b[1] &&
                    edge.b[0] === requiredEdge.a[0] &&
                    edge.b[1] === requiredEdge.a[1])
            return edgeFound
        })
    ...
    initializePaths = (startPoint, destination) => {
        ...
        this.currentPathfindingProgress = map(adjacentEdges, edge => ({
            ...
            reachedDestination:
                edge.b[0] === destination[0] && edge.b[1] === destination[1],
        }))
    }

    findAdjacentEdges = startPoint => {
        const adjacentEdges = filter(
            this.edges,
            edge =>
